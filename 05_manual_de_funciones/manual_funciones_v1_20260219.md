# MANUAL DE FUNCIONES — NVC
## Versión 1.2 | Nutrición, Vacunas y Consultoría

---

| Campo              | Detalle                                         |
|--------------------|--------------------------------------------------|
| **Empresa**        | NVC — Nutrición, Vacunas y Consultoría          |
| **Documento**      | Manual de Funciones v1.2                        |
| **Fecha**          | 7 de abril de 2026                              |
| **Elaboró**        | Consultor Externo                                |
| **Estatus**        | Borrador — Pendiente validación con DG          |
| **Revisión**       | Cada 6 meses o ante cambios organizacionales    |

---

## ÍNDICE

1. [Introducción y propósito](#1-introducción-y-propósito)
2. [Instrucciones de uso](#2-instrucciones-de-uso)
3. [Estructura de cada descripción de puesto](#3-estructura-de-cada-descripción-de-puesto)
4. [Puestos — Nivel Dirección](#4-puestos--nivel-dirección)
5. [Puestos — Gerencia Administrativa](#5-puestos--gerencia-administrativa)
6. [Puestos — Gerencia Comercial](#6-puestos--gerencia-comercial)
7. [Puestos — Gerencia Técnica](#7-puestos--gerencia-técnica)
8. [Puestos — Marketing](#8-puestos--marketing)
9. [Puestos — Operativos (bodega y logística)](#9-puestos--operativos-bodega-y-logística)
10. [Política de evaluación del desempeño](#10-política-de-evaluación-del-desempeño)
11. [Registro y firmas](#11-registro-y-firmas)

---

## 1. INTRODUCCIÓN Y PROPÓSITO

El **Manual de Funciones** de NVC es el documento que define con precisión las responsabilidades, funciones, métricas de desempeño y relaciones jerárquicas de cada puesto dentro de la organización.

### Propósitos del manual:
- Eliminar ambigüedad en las responsabilidades ("eso no es mi trabajo")
- Facilitar la inducción de nuevo personal
- Servir de base para evaluación de desempeño y bonos
- Ser el fundamento del manual de apertura de nuevas sucursales
- Documentar el conocimiento organizacional (reducir dependencia de personas clave)

### Alcance:
Aplica a todos los colaboradores de NVC: Apatzingán (matriz), San Simón, Nueva Italia, sucursales 4 y 5, y equipo itinerante.

---

## 2. INSTRUCCIONES DE USO

- Cada colaborador debe recibir una copia de la descripción de su puesto y firmarla de recibido
- El encargado de sucursal conserva copia de las descripciones de todos los puestos a su cargo
- Vianey conserva los originales firmados en el expediente de cada empleado
- Ante cualquier duda sobre responsabilidades, este manual es el documento de referencia

---

## 3. ESTRUCTURA DE CADA DESCRIPCIÓN DE PUESTO

Cada puesto contiene:
1. **Identificación:** nombre, área, nivel jerárquico, a quién reporta, a quién supervisa
2. **Objetivo del puesto:** razón de ser del rol
3. **Funciones y responsabilidades:** listado detallado
4. **KPIs (indicadores de desempeño):** cómo se mide el éxito
5. **Perfil requerido:** educación, experiencia, habilidades
6. **Prohibiciones expresas**
7. **Firma de recibido**

---

## 4. PUESTOS — NIVEL DIRECCIÓN

---

### PUESTO: DIRECTOR GENERAL

| Campo | Detalle |
|-------|---------|
| **Área** | Dirección |
| **Nivel** | N1 |
| **Reporta a** | Socios / Asamblea |
| **Supervisa a** | Gerentes de área |
| **Horario** | Estratégico — flexible |

#### Objetivo del puesto
Dirigir la estrategia de crecimiento de NVC, asegurar la rentabilidad del negocio, representar a la empresa ante clientes clave, proveedores y autoridades, y tomar decisiones de alto impacto.

#### Funciones y responsabilidades

**Estrategia y negocio:**
- Definir la dirección estratégica anual de la empresa
- Aprobar presupuestos, inversiones y nuevas contrataciones
- Establecer metas por área y sucursal
- Representar a NVC ante Purina y otros proveedores estratégicos
- Negociar contratos, condiciones comerciales y alianzas

**Gestión de clientes clave:**
- Atender y visitar a los clientes de mayor volumen (cuentas clave)
- Resolver escalonamiento de problemas técnicos complejos
- Desarrollar nuevas relaciones comerciales estratégicas

**Asesoría técnica (transitoria):**
- Brindar asesoría en vacunación y manejo animal hasta que el promotor técnico esté operativo
- Apoyar en planes de alimentación para clientes estratégicos

**Gestión de resultados:**
- Presidir la reunión semanal de resultados (viernes)
- Revisar el dashboard gerencial semanal
- Aprobar o rechazar decisiones de crédito > $5,000

#### KPIs del Director General

| Indicador | Meta | Frecuencia |
|-----------|------|-----------|
| Crecimiento en ventas totales | +15% anual | Mensual |
| Nuevas sucursales abiertas | 2 en 12 meses | Anual |
| % tiempo en tareas estratégicas | >60% | Trimestral |
| % tiempo en tareas operativas | <20% (meta) | Trimestral |
| Retención de clientes clave | >90% | Trimestral |

#### Perfil requerido
- Propietario y fundador de la empresa
- Especialización técnica en vacunación animal y nutrición pecuaria
- Capacidad de negociación y liderazgo

#### Prohibiciones
- No debe realizar cotizaciones rutinarias sin delegar primero al área correspondiente
- No debe resolver problemas operativos que corresponden a encargados (una vez el sistema esté en marcha)

---

## 5. PUESTOS — GERENCIA ADMINISTRATIVA

---

### PUESTO: GERENTE ADMINISTRATIVO (Formalizar con Vianey)

| Campo | Detalle |
|-------|---------|
| **Área** | Administrativa |
| **Nivel** | N2 |
| **Reporta a** | Director General |
| **Supervisa a** | Cajero/a, Auxiliar Contable, Bodegueros (en función admin) |
| **Horario** | Lunes a viernes, 8:00–17:00; sábado 8:00–13:00 |

#### Objetivo del puesto
Asegurar el control financiero, administrativo y contable de la empresa, garantizando el registro correcto de operaciones, el flujo de caja sano y el cumplimiento de obligaciones fiscales y laborales.

#### Funciones y responsabilidades

**Administración financiera y tesorería:**
- Administrar el dinero y flujo de efectivo de la empresa
- Ingresar compras en el sistema (MyBusiness Pro)
- Verificar y realizar ajustes en el sistema
- Realizar conciliación bancaria mensual (compras/gastos)
- Verificar clientes, cortes de caja y transferencias recibidas
- Realizar trámites bancarios presenciales (ir al banco)
- Pagar servicios periódicos: luz, renta, agua, predial, internet

**Compras y pedidos:**
- Recibir reporte de faltantes de cada sucursal
- Emitir órdenes de compra a proveedores (Purina y otros)
- Dar seguimiento a entregas y verificar facturas de proveedor
- Negociar condiciones de pago con proveedores rutinarios

**Inventario y mermas:**
- Ajustar inventario directamente en el sistema
- Gestionar pendientes de inventario en el sistema
- Registrar y controlar mermas
- Pasar información consolidada de inventario al Director General

**Facturación y contabilidad:**
- Coordinar la facturación CFDI (apoyada por su auxiliar)
- Conciliar MyBusiness vs CONTPAQ
- Generar reporte de cartera vencida cada viernes
- Coordinar con el contador externo para declaraciones fiscales
- Elaborar reportes e informes de compras para el contador

**Control administrativo:**
- Administrar y actualizar el catálogo de clientes y proveedores en MyBusiness
- Administrar y hacer cumplir las políticas internas de la empresa
- Custodiar contratos y documentos legales de la empresa
- Controlar el archivo físico y digital de facturas
- Manejo básico de MyBusiness Pro

**Gestión de personal (RRHH):**
- Responsable directa del personal de la empresa
- Procesar altas y bajas de personal ante IMSS
- Gestionar nómina y presentarla al DG para aprobación
- Supervisar el cumplimiento de horarios y asistencia

**Reporte y comunicación:**
- Generar reporte financiero mensual para el DG
- Elaborar reportes e informes de compras
- Comunicación directa con el Gerente General
- Integrar datos al dashboard gerencial (ventas, cartera, flujo)
- Alertar al DG sobre cualquier irregularidad financiera

#### KPIs del Gerente Administrativo

| Indicador | Meta | Frecuencia |
|-----------|------|-----------|
| Facturas emitidas el mismo día | 100% | Diario |
| Conciliaciones al día | Sin rezago mayor a 1 semana | Semanal |
| Cartera vencida / ventas totales | <10% | Mensual |
| Declaraciones fiscales a tiempo | 100% | Mensual |
| Personal con IMSS al corriente | 100% | Mensual |

#### Perfil requerido
- Licenciatura en Administración, Contabilidad, Psicología Organizacional o afín
- Experiencia mínima: 3 años en funciones administrativas
- Dominio de MyBusiness y CONTPAQ
- Organización, discreción y capacidad para manejar múltiples tareas

#### Prohibiciones
- No otorgar crédito a clientes sin autorización del DG
- No realizar contrataciones sin aprobación del DG
- No compartir información financiera con personal no autorizado

---

### PUESTO: RESPONSABLE DE LOGÍSTICA Y CARTERA

| Campo | Detalle |
|-------|---------|
| **Área** | Administrativa / Operativa |
| **Nivel** | N3 |
| **Reporta a** | Gerente Administrativo (Vianey) |
| **Supervisa a** | Coordinación con bodegueros y repartidores para logística |
| **Titular actual** | Rossy |
| **Horario** | Lunes a viernes, 8:00–17:00; sábado 8:00–13:00 |

> **Historial del puesto:**
> - **Feb 2026 (Diagnóstico 01):** Rossy operaba como cajera en Apatzingán.
> - **Abr 2026 (v1.1):** Reasignada como Auxiliar Administrativo de Vianey.
> - **Abr 2026 (v1.2):** Puesto formalizado como **Responsable de Logística y Cartera**, con autonomía propia en gestión de pedidos, análisis de cartera de clientes e inventarios.

#### Objetivo del puesto
Asegurar la operación logística y comercial del negocio: programar y dar seguimiento a pedidos, mantener actualizada la cartera de clientes (activos, en riesgo y vencida), y supervisar el estado del inventario en todas las sucursales. Actúa como enlace operativo entre el área administrativa y las sucursales.

#### Funciones y responsabilidades

**Logística y programación de pedidos:**
- Programar y coordinar los pedidos a proveedores (Purina y otros) según necesidades de inventario
- Dar seguimiento a órdenes de compra hasta su recepción y confirmación
- Coordinar traspasos de mercancía entre sucursales en el sistema
- Coordinar con encargados de sucursal las necesidades de reabastecimiento
- Apoyar en la coordinación de entregas a clientes cuando se requiera

**Cartera de clientes:**
- Mantener actualizado el directorio/catálogo de clientes en el sistema
- Identificar y reportar clientes con mayor volumen de compra (top buyers)
- Detectar clientes que han dejado de comprar (inactivos >30 días) y alertar al DG o Gerente Comercial
- Gestionar y reportar la cartera vencida: clientes con crédito no pagado
- Realizar seguimiento de cobranza con clientes: llamadas, WhatsApp, acuerdos de pago
- Verificar que las notas pagadas se den de baja en el sistema el mismo día

**Control de inventario:**
- Revisar el estatus de inventario en todas las sucursales con frecuencia semanal
- Detectar diferencias entre inventario físico y sistema, reportarlas a Vianey
- Realizar o coordinar el inventario físico de apoyo
- Registrar traspasos entre sucursales en el sistema (MyBusiness)

**Facturación:**
- Emitir facturas CFDI cuando se requiera
- Coordinar con Vianey el proceso de facturación

**Bitácora de trabajo diaria:**
- Registrar diariamente las actividades realizadas: pedidos gestionados, clientes contactados, cobranzas, inventarios revisados
- La bitácora puede llevarse en libreta o formato digital (lista sencilla)
- Disponible para revisión por Vianey o el DG en cualquier momento

**Reportes:**
- Enviar reporte semanal de cartera: clientes activos, inactivos y vencida
- Reportar estatus de pedidos y entregas pendientes
- Participar en la reunión semanal de viernes

#### KPIs del Responsable de Logística y Cartera

| Indicador | Meta | Frecuencia |
|-----------|------|-----------|
| Pedidos a proveedores sin errores | 100% | Por pedido |
| Traspasos registrados en sistema | 100% el mismo día | Diario |
| Clientes inactivos >30 días identificados y reportados | 100% detectados | Semanal |
| Cartera vencida reportada al DG | Sin rezago mayor a 1 semana | Semanal |
| Notas pagadas dadas de baja en sistema | 100% el mismo día | Diario |
| Bitácora de trabajo al día | 100% de días hábiles | Diario |
| Reporte semanal de cartera entregado | 100% viernes | Semanal |

#### Perfil requerido
- Preparatoria concluida (mínimo); deseable estudios en administración o contabilidad
- Experiencia en funciones administrativas y trato con clientes
- Manejo de MyBusiness Pro (o capacitación inmediata)
- Capacidad de organización, seguimiento y comunicación proactiva
- Honestidad, responsabilidad y discreción en el manejo de información de clientes

#### Prohibiciones
- No otorgar o ampliar crédito a clientes sin autorización de Vianey o el DG
- No realizar ajustes de inventario sin respaldo físico y supervisión de Vianey
- No compartir información de cartera o clientes con personal no autorizado
- No comprometer entregas o pedidos sin verificar disponibilidad y aprobación

---

### PUESTO: CAJERO/A *(por confirmar titular en Apatzingán — Diagnóstico 02)*

| Campo | Detalle |
|-------|---------|
| **Área** | Comercial / Sucursal |
| **Nivel** | N4 |
| **Reporta a** | Encargado de Sucursal |
| **Supervisa a** | Ninguno directamente |
| **Titular actual** | Por confirmar en Apatzingán (Diagnóstico 02) |
| **Horario** | Según horario de sucursal |

> **Nota (v1.1):** Con Rossy reasignada como Auxiliar Administrativo, se debe confirmar quién opera la caja en la sucursal Apatzingán. Este puesto se mantiene como perfil genérico replicable para todas las sucursales.

#### Objetivo del puesto
Registrar con exactitud todas las transacciones de venta en el sistema, cobrar correctamente y realizar el corte de caja diario sin diferencias.

#### Funciones y responsabilidades
- Atender al cliente en el punto de cobro con amabilidad y eficiencia
- Registrar cada venta en MyBusiness (o POS una vez implementado)
- Cobrar en efectivo, transferencia o tarjeta según el caso
- Emitir ticket o factura al cliente
- Realizar el corte de caja al cierre, comparar efectivo vs sistema
- Reportar faltantes de producto al encargado para reposición
- Reportar diferencias de caja inmediatamente al encargado
- Custodiar el efectivo del día hasta el depósito o entrega al DG
- **Llenar la bitácora de trabajo diaria** (ver política de bitácora más abajo)

**Bitácora de trabajo diaria (Cajero/a):**
Registrar al final de cada turno, en libreta o computadora, la siguiente información básica:
1. Fecha y turno
2. Total de ventas del día (monto sistema)
3. Corte de caja: resultado (cuadrado / diferencia y monto)
4. Incidencias del día: clientes con problemas, faltantes de producto, pagos no registrados, etc.
5. Nombre y firma del cajero/a

#### KPIs del Cajero/a

| Indicador | Meta | Frecuencia |
|-----------|------|-----------|
| Diferencia en corte de caja | $0 (tolerancia: $50 por día) | Diario |
| Ventas registradas vs cobradas | 100% coincidencia | Diario |
| Tickets emitidos sin error | 99%+ | Diario |
| Bitácora de trabajo al día | 100% de días con turno | Diario |

#### Perfil requerido
- Preparatoria concluida
- Experiencia previa en caja o atención al cliente (deseable)
- Manejo básico de computadora
- Honestidad, responsabilidad y agilidad numérica

#### Prohibiciones
- No prestar dinero de la caja bajo ninguna circunstancia
- No registrar ventas fuera del sistema
- No otorgar descuentos sin autorización del encargado

---

## 6. PUESTOS — GERENCIA COMERCIAL

---

### PUESTO: ENCARGADO/A DE SUCURSAL

| Campo | Detalle |
|-------|---------|
| **Área** | Comercial |
| **Nivel** | N3 |
| **Reporta a** | Gerente Comercial / DG (transitorio) |
| **Supervisa a** | Todo el personal de su sucursal |
| **Horario** | Según horario de sucursal; disponible en emergencias |

#### Objetivo del puesto
Garantizar la operación diaria correcta de la sucursal a su cargo: control de inventario, cumplimiento de metas de venta, atención al cliente, control del equipo y reporte a la gerencia.

#### Funciones y responsabilidades

**Apertura y cierre:**
- Verificar que las instalaciones estén en condiciones para operar
- Activar los sistemas al inicio del día
- Supervisar el cierre y que el efectivo quede resguardado

**Ventas y atención al cliente:**
- Supervisar y apoyar la atención en mostrador
- Asegurar que ningún cliente se vaya sin atención (protocolo anti-pérdida)
- Registrar en base de datos a clientes que no compraron (seguimiento)
- Coordinar entregas a domicilio con el equipo de logística

**Control de inventario:**
- Verificar inventario diario de productos A (alta rotación)
- Supervisar conteo semanal completo
- Reportar diferencias inmediatamente a Vianey
- Recibir mercancía y verificar contra factura del proveedor

**Gestión del equipo:**
- Supervisar asistencia y puntualidad (libro de registro)
- Dirigir las actividades diarias del personal a cargo
- Autorizar permisos de hasta 1 día; turnar al DG los de más de 1 día
- Reportar incidencias disciplinarias a Vianey / DG

**Reporte diario:**
- Enviar al grupo de WhatsApp Business: corte de caja, resumen de ventas, diferencias de inventario, novedades
- Participar en la reunión semanal de viernes

#### KPIs del Encargado/a de Sucursal

| Indicador | Meta | Frecuencia |
|-----------|------|-----------|
| Ventas de la sucursal vs meta | ≥100% de meta mensual | Mensual |
| Diferencias en inventario | <2% del valor total | Semanal |
| Cortes de caja sin diferencia | ≥90% de días | Mensual |
| Clientes atendidos registrados | ≥80% con nombre en base de datos | Semanal |
| Reportes diarios enviados | 100% de días hábiles | Diario |

#### Perfil requerido
- Preparatoria o licenciatura (preferente)
- Experiencia en ventas o atención al cliente
- Habilidad de liderazgo básico y toma de decisiones
- Manejo básico de MyBusiness (se capacita)

#### Prohibiciones
- No otorgar crédito sin autorización del DG
- No modificar inventario en sistema sin respaldo físico
- No contratar o despedir personal sin autorización del DG

---

### PUESTO: PROMOTOR DE VENTAS

| Campo | Detalle |
|-------|---------|
| **Área** | Comercial |
| **Nivel** | N4 |
| **Reporta a** | Gerente Comercial / DG |
| **Supervisa a** | Ninguno |
| **Horario** | Campo: lunes a viernes; reporte: viernes |

#### Objetivo del puesto
Generar nuevas ventas, visitar clientes en campo, levantar pedidos y recuperar clientes inactivos para incrementar el volumen de toneladas vendidas.

#### Funciones y responsabilidades
- Prospectar y visitar productores (acuicultores, ganaderos, avicultores) en la zona asignada
- Levantar pedidos en campo y registrarlos en el sistema
- Recuperar clientes que no han comprado en los últimos 90 días
- Presentar el catálogo de productos y ofrecer asesoría básica
- Coordinar con el encargado de sucursal la logística de entrega
- Entregar reporte semanal de visitas, pedidos levantados y clientes prospectados
- Apoyar en la gestión de cobranza de clientes en campo

#### Compensación
- Sueldo base + **$400 pesos por tonelada vendida** (solo ventas registradas en sistema)

#### KPIs del Promotor de Ventas

| Indicador | Meta | Frecuencia |
|-----------|------|-----------|
| Visitas semanales | ≥20 visitas | Semanal |
| Clientes nuevos captados | ≥5 por mes | Mensual |
| Clientes inactivos reactivados | ≥5 por trimestre | Trimestral |
| Toneladas gestionadas | Según meta por zona | Mensual |
| Reporte semanal entregado | 100% viernes | Semanal |

#### Perfil requerido
- Preparatoria o licenciatura en ventas, administración o agropecuario
- Experiencia en ventas de campo (sector agropecuario deseable)
- Conocimiento de la región y zona de productores
- Licencia de manejo vigente
- Capacidad de comunicación y persuasión

#### Prohibiciones
- No ofrecer precios distintos a la lista autorizada sin consultar al DG
- No cobrar efectivo sin entregar recibo y registrar en sistema
- No comprometer entregas que no hayan sido coordinadas con logística

---

## 7. PUESTOS — GERENCIA TÉCNICA

---

### PUESTO: PROMOTOR TÉCNICO (Biólogo / Médico Veterinario)

| Campo | Detalle |
|-------|---------|
| **Área** | Técnica |
| **Nivel** | N4 |
| **Reporta a** | Director General / Gerente Técnico |
| **Supervisa a** | Ninguno |
| **Horario** | Mixto: campo y oficina |

#### Objetivo del puesto
Brindar asesoría técnica especializada a clientes en planes de alimentación, manejo sanitario y solución de problemas productivos, liberando al Director General de esta función y agregando valor técnico diferenciado.

#### Funciones y responsabilidades
- Elaborar y dar seguimiento a planes de alimentación para clientes
- Visitar granjas y unidades productivas para diagnóstico técnico
- Resolver consultas técnicas de clientes por WhatsApp, teléfono o visita
- Apoyarse en herramientas de IA para recomendaciones técnicas actualizadas
- Brindar capacitación técnica a los encargados de sucursal y promotores de ventas
- Apoyar en la selección y recomendación de vacunas, antibióticos y suplementos
- Presentar informes técnicos al DG y proponer nuevos productos o servicios

#### KPIs del Promotor Técnico

| Indicador | Meta | Frecuencia |
|-----------|------|-----------|
| Clientes con plan de alimentación activo | ≥20 en primer trimestre | Trimestral |
| Consultas técnicas resueltas en <24h | ≥90% | Mensual |
| Visitas técnicas a campo | ≥3 por semana | Semanal |
| Clientes retenidos gracias a soporte técnico | Medición trimestral | Trimestral |

#### Perfil requerido
- Licenciatura en Biología, Medicina Veterinaria y Zootecnia o afín
- Cédula profesional
- Experiencia en acuacultura o producción animal (indispensable)
- Capacidad de comunicación técnica con productores
- Disposición para uso de herramientas digitales e IA

---

### PUESTO: TÉCNICO VACUNADOR (Itinerante)

| Campo | Detalle |
|-------|---------|
| **Área** | Técnica |
| **Nivel** | N5 |
| **Reporta a** | Director General / Promotor Técnico |
| **Sucursales que apoya** | Apatzingán, San Simón, Nueva Italia |

#### Objetivo del puesto
Ejecutar procesos de vacunación animal (peces, ganado, aves) según protocolos definidos, apoyando al Director General y al equipo técnico.

#### Funciones y responsabilidades
- Realizar vacunaciones según agenda coordinada por el DG
- Mantener en orden el equipo de vacunación
- Registrar las vacunaciones realizadas (registro diario)
- Reportar incidencias o reacciones a vacunas al DG / Promotor Técnico
- Apoyar en la venta de insumos técnicos (vacunas, antibióticos) en campo

---

## 8. PUESTOS — MARKETING

---

### PUESTO: RESPONSABLE DE MARKETING (Formalizar con Julio)

| Campo | Detalle |
|-------|---------|
| **Área** | Marketing |
| **Nivel** | N3 |
| **Reporta a** | Director General |
| **Coordina con** | Encargados, Promotores, Agencia/Freelancer |
| **Horario** | Lunes a viernes + soporte digital |

#### Objetivo del puesto
Posicionar la marca NVC en el mercado digital y local, generar contenido de valor, administrar los canales de comunicación y apoyar las campañas comerciales de las sucursales.

#### Funciones y responsabilidades
- Administrar las redes sociales oficiales de NVC (Facebook, Instagram)
- Publicar contenido de acuerdo al calendario semanal aprobado
- Gestionar WhatsApp Business institucional
- Coordinar con encargados de sucursal las promociones y ofertas a difundir
- Elaborar propuestas de campañas digitales para eventos y lanzamientos
- Apoyar en el estudio de mercado para nuevas zonas (Michoacán)
- Generar reportes mensuales de alcance, interacciones y leads en redes

#### KPIs del Responsable de Marketing

| Indicador | Meta | Frecuencia |
|-----------|------|-----------|
| Publicaciones en redes | 3 posts + 5 stories/semana | Semanal |
| Crecimiento seguidores | +10% mensual | Mensual |
| Mensajes respondidos en WhatsApp Business | <2h de respuesta | Diario |
| Leads generados desde redes | ≥5 por mes | Mensual |

---

## 9. PUESTOS — OPERATIVOS (BODEGA Y LOGÍSTICA)

---

### PUESTO: BODEGUERO / AUXILIAR DE INVENTARIO

| Campo | Detalle |
|-------|---------|
| **Área** | Operativa |
| **Nivel** | N5 |
| **Reporta a** | Encargado de Sucursal |
| **Titulares actuales** | José Galván (Apatzingán), Álvaro (Apatzingán), David (Nueva Italia) |

#### Objetivo del puesto
Mantener el orden, limpieza y control del almacén, asegurar que la mercancía esté correctamente registrada y disponible para venta y entrega.

#### Funciones y responsabilidades
- Descarga y carga de mercancía
- Verificar cantidades y estado físico al recibir mercancía (contra factura)
- Ordenar y rotular productos en almacén
- Realizar conteo físico de inventario (diario de productos A, semanal completo)
- Mantener limpias las instalaciones: almacén, baño, área de ventas
- Apoyar al encargado en el reporte de diferencias de inventario
- Cargar vehículos para rutas de entrega

#### KPIs del Bodeguero

| Indicador | Meta | Frecuencia |
|-----------|------|-----------|
| Diferencias en recepción de mercancía reportadas | 100% de casos | En cada recepción |
| Conteo de inventario completado | 100% de días asignados | Diario/Semanal |
| Instalaciones limpias y ordenadas | Supervisión encargado | Diario |

---

### PUESTO: REPARTIDOR / AUXILIAR DE LOGÍSTICA

| Campo | Detalle |
|-------|---------|
| **Área** | Logística |
| **Nivel** | N5 |
| **Reporta a** | Encargado de Sucursal |
| **Titulares actuales** | Pepe (San Simón), Uriel (San Simón), Julio (Nueva Italia — también inventarios) |

#### Objetivo del puesto
Ejecutar las entregas de producto a clientes en campo de manera puntual y ordenada, cobrar y reportar al encargado o cajero.

#### Funciones y responsabilidades
- Cargar el vehículo según la ruta del día
- Entregar pedidos en tiempo y en condiciones acordadas
- Cobrar en campo (efectivo o confirmar transferencia)
- Emitir o entregar nota de remisión firmada por el cliente
- Reportar al encargado o cajero el resultado de cada entrega y cobro
- Mantener el vehículo en buen estado y reportar cualquier falla mecánica

#### Prohibiciones
- No entregar producto sin orden de despacho autorizada
- No cobrar efectivo sin emitir comprobante
- No realizar entregas no registradas en el sistema

---

## 10. POLÍTICA DE EVALUACIÓN DEL DESEMPEÑO

### Frecuencia de evaluaciones
- **Mensual:** Revisión de KPIs por encargado y gerencia
- **Trimestral:** Evaluación formal de desempeño con retroalimentación
- **Anual:** Evaluación integral con decisión de continuidad, ajuste salarial o promoción

### Escala de evaluación

| Calificación | Descripción | Consecuencia |
|-------------|-------------|--------------|
| Excelente (90–100%) | Supera todas las metas | Bono de desempeño |
| Bueno (75–89%) | Cumple la mayoría de metas | Reconocimiento verbal |
| Regular (60–74%) | Cumple parcialmente | Plan de mejora 30 días |
| Insuficiente (<60%) | No cumple metas | Advertencia escrita / proceso disciplinario |

### Proceso disciplinario básico
1. Primera falta leve: llamada de atención verbal
2. Segunda falta leve / primera grave: advertencia escrita
3. Tercera falta leve / segunda grave: suspensión temporal
4. Falta muy grave (robo, manipulación de inventario, venta fuera del sistema): terminación inmediata

---

## 11. REGISTRO Y FIRMAS

Cada colaborador debe firmar la descripción de su puesto, indicando que ha leído, comprendido y acepta las funciones y responsabilidades asignadas.

| Nombre del colaborador | Puesto | Sucursal | Firma | Fecha |
|-----------------------|--------|----------|-------|-------|
| | | | | |
| | | | | |

**Custodio del documento:** Vianey — Área Administrativa
**Siguiente revisión:** Agosto 2026

---

### Historial de versiones

| Versión | Fecha | Cambios | Elaboró |
|---------|-------|---------|---------|
| 1.0 | 19/02/2026 | Versión inicial — basada en Diagnóstico 01 | Consultor externo |
| 1.1 | 05/04/2026 | Actualización funciones Vianey (entrevista directa). Reasignación de Rossy de "Cajera" a "Auxiliar Administrativo". Nuevo puesto de Cajero/a genérico. Incorporación de funciones: mermas, ajuste de inventario, conciliación bancaria mensual, trámites bancarios, responsable de personal | Consultor externo |
| 1.2 | 07/04/2026 | Formalización del puesto de Rossy como **Responsable de Logística y Cartera** (N3): nuevas funciones de programación de pedidos, análisis de cartera de clientes (activos, inactivos, vencida) y revisión de inventarios en sucursales. Incorporación de **bitácora de trabajo diaria** obligatoria para todos los puestos administrativos y cajeros. | Consultor externo |

---

*NVC — Manual de Funciones v1.2 | Consultoría Externa | Confidencial*
